<!--
  function: 处理事项里面的图片上传
  author  : wq
  update  : 2018/7/19 17:34
-->
<template>
  <div class="data-upload-box">
    <div class="back-box" v-if="!openChoose">
      <div class="title-wrap"><text class="back-title">{{title}}</text></div>
      <div class="icon-wrap"  @click="doChooseImg" v-if="uploadImageLength > 0" :key="`data-upload-${new Date().getTime()}-choose`">
    ...
    <div class="back-box" v-else>
      <div class="title-wrap"><text class="back-title">{{title}}</text></div>
      <div :style="{opacity: chooseNum && 1 || 0.3}" class="icon-wrap"  @click="doDeleteImage">
        <d-image src="/image/icon-delete.png" width="32" height="38"></d-image>
        <text class="back">删除</text>
      </div>
      <div class="icon-wrap"  @click="doCancelChoose">
        ...
        this.requestApi.data_menu_list({
          ...
          success: (data) => {
            ...
            native_module_events(DEFINE_IMAGE_URL, urls, 1).then((data) => {
              this.imgUploadList = list.map(item => Object.assign({}, item, {src: data[item.fileId], fileId: item.fileId}));
