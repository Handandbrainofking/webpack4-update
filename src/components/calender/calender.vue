...
import WxcMinibar from '@/components/wxc/minibar.vue'
import { isWeb, pageTransitionAnimationStyle, isIPhoneX, pageTransitionAnimation, screenHeight } from '@/utils/utils'
...
export default {
  ...
  computed: {
    calendarExtendStyle() {
      return pageTransitionAnimationStyle(this.animationType)
    },
    monthsArray() {
  ...
  methods: {
    ...
    scrollToDate() {
      setTimeout(() => {
        if (this.departDate) {
          const el = this.$refs.departDate && this.$refs.departDate[0];
    ...
    _animate(status, callback = null) {
      ...
      if (this.animationType === 'push') {
        pageTransitionAnimation(ref, `translateX(${status ? -750 : 750}px)`, status, callback)
      } else if (this.animationType === 'model') {
        pageTransitionAnimation(ref, `translateY(${status ? -screenHeight : screenHeight}px)`, status, callback)
      }
    },
    show() {
      const { needDestroy } = this;
