...
abstract class BaseWidget implements Widget {
  ...
  // Reconsider the field parameter in this class and the operation during draw(); Make a CPU/Memory balance.
  // use float to avoid 1px problem
  ...
  public void setBackgroundAndBorder(@NonNull BorderDrawable backgroundBorder) {
    // Change the code of BorderDrawable is more appropriate as it draws the borderLine from (0,0) not from getBounds
    // If the above is finished, no more traslate in draw in needed, only clip is enough.
